...
\title{\textsc{Syst\`emes dynamiques} \\Corrig\'e DM n°3}
\date{}
\author{Bas\'e sur la copie de Manh-Linh Nguyen}

\begin{document}
{\noindent \'Ecole Normale Sup\'erieure  \hfill \hfill  \texttt{chaubet\at dma.ens.fr}}
\\
{2020/2021 \hfill}
...
\begin{enumerate}
    ...
    \item \label{Partie13}
        \begin{enumerate}
            ...
            i.e. $T^{-m}(I_{b_{m+1}}) \cap I_{b_1,\ldots,b_m} = I_{b_1,\ldots,b_{m+1}}$. En appliquant {\bf \ref{Partie8}.}, on a
                ...
                $$
                \begin{aligned}
                \ell(E_{m+1}(\abf)^c \cap I_{b_1, \dots, b_m}) &\leq \ell(I_{b_1, \dots, b_m})\left(1 - 2\ell(E_{m+1}(\abf)\right) \\
                &\leq \ell(I_{b_1, \dots, b_m}) \left(1 - \frac{C}{a_{m+1} +1}\right),
                \end{aligned}
                $$
    ...
    \item \label{Partie14}
        \begin{enumerate}
            ...
            \item \label{Partie14b} On a n\'ecessairement $f(q) > 0$ pour tout $q$ (s'il existe $q_0$ tel que $f(q_0) = 0$, comme $(qf(q))_{q}$ est d\'ecroissante, on aura $f(q) = 0$ pour tout $q \ge q_0$, ce qui contredit le fait que $\sum_q f(q)$ diverge). Montrons que la s\'erie $\sum_n \varphi(n)$ diverge. On a, par décroissance de $(qf(q))_q$,
                            $$\sum_{q \ge 1} f(q) = \sum_{n \ge 0} \sum_{q = 4^n}^{4^{n+1} - 1} f(q) = \sum_{n \geq 0} \sum_{q = 4^n}^{4^{n+1} - 1} \frac{qf(q)}{q} \leqslant \sum_{n\geq 0}  \varphi(n) \sum_{q = 4^n}^{4^{n+1} - 1} \frac{1}{q} \leq 3\sum_{n \ge 0}\varphi(n).$$
            Il suit de {\bf \ref{Partie13b}.} que pour presque tout $x$, $a_{n+1}(x) > \frac{1}{\varphi(n)}$ infiniment souvent. Pour tel $x$ et $n$, par (\ref{Partie4c})
