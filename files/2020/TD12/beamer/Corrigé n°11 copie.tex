...
$$
en identifiant une alg\`ebre avec l'ensemble de ses atomes. 
...
\textbf{2.} On note $\Qcal = \{Q_1, \dots, Q_\ell\}$ et $q_j = \mu(Q_j).$  Alors
$$
...
\textbf{2.} Soit $\Pcal$ une partition. On note $\Pcal^n_f = \bigvee_{j=0}^n f^{-j}(\Pcal)$.  Alors
$$
\begin{aligned}
...
\textbf{2.} On d\'ecoupe le cercle en une union d'intervalles $S^1 = I_1 \sqcup \cdots \sqcup I_n$ avec $\mathrm{diam}(I_j) < 1/n$.  
...
$$
par l'\textbf{Exercice 2}. Ainsi (\ref{eq:1}) implique $h_\mu(f) = h_\mu(f, \Pcal)$. 

Puisque $\Pcal^k_f \leqslant \Pcal^\ell_f$ pour tous $k \leqslant \ell$, on a que $(\diam \Pcal^n_f)_n$ d\'ecro”t. 

Supposons que $\lim_n \diam \Pcal^n_f = \varepsilon > 0$. Alors pour tout $n$ on a $\diam \Pcal^n_f \geqslant \varepsilon$.  
...
Puisque $\diam \Pcal^n_f \geqslant \varepsilon$, on peut trouver $x_n, y_n \in X$ et $P\in \Pcal^n_f$ tels que $\dist(x_n, y_n) \geqslant \varepsilon / 2$ et $x_n, y_n \in P$.  
...
{Exercice 6}
\textbf{1.} On a $H_\mu(\Pcal \vee \Qcal) = H_\mu(\Qcal) + H_\mu(\Pcal|\Qcal)$ pour tous $\Pcal, \Qcal$.  Donc 
$$
\begin{aligned}
