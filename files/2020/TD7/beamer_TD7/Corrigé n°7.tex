...
$$
o\`u $1 < \mu < \lambda^{-1}$ et $N \geqslant 1$. %\pause 
Alors 
$$
\begin{aligned}
...
$$
o\`u $\pi_{s/u}(x)$ est la projection $T_xM \to E^{s/u}(x)$. 

%\pause
\vspace{0.2cm}

Puisque $\pi_s(x)$ et $\pi_u(x)$ d\'ependent continument de $x$ (car $E^s(x)$ et $E^u(x)$ d\'ependent continument de $x$), la norme $\|\cdot\|'$ est continue.
...
\textbf{1.} Soit $x$ tel que $f^n(x) = x$ ; on pose $g = f^n$. Alors $A = (\dd g)_x : T_x M \to T_xM$. Supposons par l'absurde qu'il existe $\lambda \in \mathrm{sp}((\dd g)_x)$ avec $|\lambda| = 1$.
...
\begin{frame}
\textbf{2.} (a) Il suffit de remplacer $x_k$ et $y_k$ par $f^{n(k)}(x_k)$ et $f^{n(k)}(y_k)$ o\`u 
$$
...
\begin{frame}
\textbf{3}. C'est une application directe de l'\textbf{Exercice 2} du TD n¡3, qui donne 
$$
...
\textbf{1.} Soit $p = (x,y) \in \R^2.$ On \'ecrit 
$$
...
On montre que $r$ et $s$ sont additifs. D'un c™t\'e on a
$$
...
\textbf{2.} Montrons d'abord que $F$ est un diff\'eomorphisme de $\R^2$. Soit $G : \R^2 \to \R^2$ qui rel\`eve $f^{-1}$. Alors on v\'erifie que $(x,y) \mapsto (G \circ F)(x,y) - (x,y)$ est \`a valeurs dans $\Z^2$, donc constante, disons \'egale \`a $(k, \ell)$. 
...
\begin{frame}
\textbf{3.} L'homotopie $F_t = tF + (1-t)A$ passe au quotient.

%\pause
\vspace{0.2cm}

\textbf{4.} On pose $p_n = a_n v + b_n w$ o\`u $Av = \lambda v$ et $A w = \lambda^{-1}$ o\`u $|\lambda| > 1.$
...
\textbf{5.} Soit $p \in \R^2$. On note $G_p : x \mapsto g(-x) - p$. 
%\pause
\vspace{0.2cm}

Alors $\|G_p(x)\| \leqslant \|g\|_\infty + \|p\|$ pour tout $x \in \R^2$. %\pause En particulier on a 
$$
...
\textbf{6.} Soit $p \in \R^2.$ On note $p_n = F^n(p)$. Alors pour tout $n \in \Z$ on a
$$
...
\textbf{7.} Soit $A \in M_2(\Z)$ de d\'eterminant $\pm1$. On note $f_A : \T^2 \to \T^2$ l'automorphisme associ\'e. Soit $\varepsilon > 0$ et $f : \T^2 \to \T^2$ tel que $\|f - f_A\|_\infty < \varepsilon$. Alors il existe un relev\'e $F$ de $f$ tel que $\|A - F\|_\infty < \varepsilon$. 
...
f_k^{\pm n_j^\pm}(y_k) - f_k^{\pm n_j^\pm}(x_k) &= \int_0^1 \left(\dd f^{\pm n_j^\pm}\right)_{(1-t)x_k + ty_k}(y_k - x_k) \dd t \\
 &  +  \int_0^1 \left(\dd f_k^{\pm n_j^\pm} - \dd f^{\pm n_j^\pm}\right)_{(1-t)x_k + ty_k}(y_k - x_k) \dd t.
\end{aligned}
$$
\end{proof}
...
Comme $t \mapsto f (\varphi_t(x))$ d\'ecro”t, on a $f(\varphi_t(x)) \geqslant f(p)$ pour tout $t$. 
